...
export async function sync(userId?: string): Promise<void> {
  ...
  await processOutbox(STORE_NAME, async (entry) => {
    ...
    if (entry.operation === 'delete') {
      await supabase.from('events').delete().eq('id', entry.payload.id as string).eq('user_id', uid);
    } else if (entry.operation === 'insert') {
      await supabase.from('events').insert([entry.payload]);
    } else if (entry.operation === 'update') {
      await supabase.from('events').update(entry.payload).eq('id', entry.payload.id as string).eq('user_id', uid);
    }
  });
